...
What is checked is free stock, not the quantity on the shelf: what is on hand less what unlapsed
[reservations](#stock-reservations) are holding, never below zero. Forty units with thirty-eight
promised is two units of stock, and buying against the forty would start the supplier's lead time
only once the last sellable unit had walked out of the door. Taking a hold and giving one back are
checked in exactly the same way as a movement, because a hold is what creates the shortage even
...
stock back up to twice the reorder threshold — a product with a threshold of 10 with 3 units free is
ordered 17, whether those 3 are all it has or all that is left unpromised of 41.
...
`quantity` is free stock — what was measured — and `reserved` says how much of what is on hand is
already promised, in the same scope: holds placed at the site for a site, holds placed anywhere for
the product total. On hand is the two added together, so the payload above describes a shelf of forty
that has two units left to sell. The log line and the email both say the free figure and the reserved
one.
...
Holding stock and giving it back are both measured on the spot, exactly as a movement is: a hold that
takes a shelf to or below its reorder point notifies the configured channels and raises the draft
order, and releasing one lets a recovered shelf fall quiet so that its next shortage is announced
afresh. See [Automatic Reordering](#automatic-reordering). Fulfilment is measured once, through the
`OUT` movement it records — the hold and the stock go together, so nothing free changes twice. A hold
that simply lapses is measured by whatever touches the product next; expiry is the passage of time,
and nothing in this path runs on a clock.
